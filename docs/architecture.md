...
| --- | --- | --- | --- |
| Analytics range plan | `VercelAnalyticsRange` in `VercelAnalyticsCore` | A selected range and clock date produce current/previous intervals, bucket cadence, and totals source. | UTC alignment, half-open windows, equal non-overlapping comparisons, bucket counts, and count-versus-aggregate intent. The Vercel adapter alone converts aggregate end times to an inclusive boundary. |
